...
Substrate_df <- data.frame(Sample = colnames(expr)) %>%
  mutate(Substrate = ifelse(str_detect(Sample, "Succinate"), "Succinate", "LMWPE"))
...
# Samples are clustered in two groups, succinate and LMWPE, as expected (no outliers).
...
# Initialize a loop to run a t.test between Succinate and LMWPE
...
me_sig_tbl <- tibble(
  Modules = str_replace(colnames(merge$newMEs), "ME", ""),
  MEpval = me_pval,
...
module_trait_pval <- data.frame(
  Substrate_padj = me_sig_tbl$MEpval.adj,
...
biofilm_prots <- c(
  custom_gene_sets[["Biofilm formation - Vibrio cholerae"]],
...
biofilm_descr <- c(
  "exopolysaccharide production protein ExoY (db=kegg)",
